...
int main(int argc, char const *argv[]) {
    ...
    start:
        ...
        if (select(sfd+1, &rfds, NULL, NULL, NULL) > 0) {
            ...
            int serv_id = get_serv_id(atoi(buf));

            if(serv_id > MAX || clients[serv_id-1] == max_client[serv_id-1]) {
                write(_sfd, "exit", 4);
                // continue;
                goto start;
            } else {
                write(_sfd, "go", 2);
                sprintf(buf, "%d", serv[serv_id-1].port);
                if(clients[serv_id-1] == 0) {
                    if(fork() == 0) {
                        execl(serv[serv_id-1].s_name, serv[serv_id-1].s_name, buf, NULL);
                        eerror("execl() error");
                    } else
                        clients[serv_id-1]++;
                }
